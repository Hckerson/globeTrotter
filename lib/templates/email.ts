...
import { APP_NAME, FRONTEND_URL } from "../../src/lib/constant";
  ...
  const footer = `
    <div class="email-footer">
      ...
      <div style="margin-top:8px; font-size:12px; color:#6c757d;">
        <a href="${FRONTEND_URL}/terms" style="color:#6c757d; text-decoration:none; margin:0 8px;">Terms</a>
        <a href="${FRONTEND_URL}/privacy" style="color:#6c757d; text-decoration:none; margin:0 8px;">Privacy</a>
        <a href="${FRONTEND_URL}/support" style="color:#6c757d; text-decoration:none; margin:0 8px;">Support</a>
        ${
          isMarketing
            ? `<a href="${FRONTEND_URL}/unsubscribe" style="color:#6c757d; text-decoration:none; margin:0 8px;">Unsubscribe</a>`
            : ""
        }
      </div>
    </div>
...
export const EmailTemplates = {
  ...
  welcomeEmail(params: { name?: string }) {
    ...
    const content = `
      ...
      <a href="${FRONTEND_URL}/dashboard" class="cta-button">Go to your dashboard</a>
    `;
    const text = `Welcome to ${APP_NAME}! Start by uploading a PDF or exploring the library: ${FRONTEND_URL}/dashboard`;
    return { subject: title, html: renderLayout(title, content), text };
  },
  ...
  loginAlert(params: { timeISO: string; ua?: string }) {
    ...
    const content = `
      ...
      <a href="${FRONTEND_URL}/reset-password" class="cta-button">Reset Password</a>
    `;
    const text = `New ${APP_NAME} login at ${dateText}. If this wasn't you, reset your password: ${FRONTEND_URL}/reset-password`;
    return { subject: title, html: renderLayout(title, content), text };
  },
};
