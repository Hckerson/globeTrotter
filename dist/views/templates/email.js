...
const { appName, frontendUrl } = config_1.config.app;
function renderLayout(title, contentHtml, options) {
    var _a;
    const isMarketing = (_a = options === null || options === void 0 ? void 0 : options.isMarketing) !== null && _a !== void 0 ? _a : false;
    const year = new Date().getFullYear();
    const footer = `
    <div class="email-footer">
      <p class="footer-text">&copy; ${year} ${appName}. All rights reserved.</p>
      <div style="margin-top:8px; font-size:12px; color:#6c757d;">
        <a href="${frontendUrl}/terms" style="color:#6c757d; text-decoration:none; margin:0 8px;">Terms</a>
        <a href="${frontendUrl}/privacy" style="color:#6c757d; text-decoration:none; margin:0 8px;">Privacy</a>
        <a href="${frontendUrl}/support" style="color:#6c757d; text-decoration:none; margin:0 8px;">Support</a>
        ${isMarketing
        ? `<a href="${frontendUrl}/unsubscribe" style="color:#6c757d; text-decoration:none; margin:0 8px;">Unsubscribe</a>`
        : ""}
      </div>
    </div>
  `;
...
<body>
  <div class="email-container">
    <div class="email-header">
      <img src="https://api.kuroconnect.com/logo-transparent.png" alt="${appName} Logo" class="logo" />
...
exports.EmailTemplates = {
    ...
    verifyEmail(params) {
        const title = `Verify your email for ${appName}`;
        const content = `
      <h2 class="greeting">Confirm your email</h2>
      <p class="message">Welcome to ${appName} — the intelligent academic second brain. Confirm your email to unlock your free credits and start uploading documents for AI-powered processing and semantic search.</p>
    ...
    `;
        const text = `Verify your email for ${appName}: ${params.verificationLink}`;
        return { subject: title, html: renderLayout(title, content), text };
    },
    // * Reset Password (link-based)
    resetPassword(params) {
        const title = `Reset your password for ${appName}`;
        const content = `
    ...
    `;
        const text = `Reset your password for ${appName}: ${params.resetLink}`;
        return { subject: title, html: renderLayout(title, content), text };
    },
    ...
    welcomeEmail(params) {
      ...
      <a href="${frontendUrl}/dashboard" class="cta-button">Go to your dashboard</a>
    `;
        const text = `Welcome to ${appName}! Start by uploading a PDF or exploring the library: ${frontendUrl}/dashboard`;
        return { subject: title, html: renderLayout(title, content), text };
    },
    // * Login Alert (security)
    loginAlert(params) {
        const title = `${appName} login alert`;
        const dateText = new Date(params.timeISO).toLocaleString();
        const content = `
      <h2 class="greeting">New login detected</h2>
      <p class="message">A sign-in to your ${appName} account was just detected.</p>
      <div style="background:#f8f9fa;border-left:4px solid #0056b3;padding:16px 20px;margin:20px 0;border-radius:0 8px 8px 0;">
      ...
      <a href="${frontendUrl}/reset-password" class="cta-button">Reset Password</a>
    `;
        const text = `New ${appName} login at ${dateText}. If this wasn't you, reset your password: ${frontendUrl}/reset-password`;
        return { subject: title, html: renderLayout(title, content), text };
    },
};
const baseStyles = `
